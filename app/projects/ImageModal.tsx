...
export default function ImageModal({ imageSrc, alt, setModalOpen }: Props) {
  console.log(alt);
  return (
    <div className="fixed z-30 w-full h-full left-0 top-0 bg-black bg-opacity-60 block overflow-hidden">
      <div className=" ">
        ...
        />
        <Image
          className="absolute top-1/2 left-1/2 z-40  object-contain overflow-hidden max-w-[98%] max-h-[98%]  opacity-100 -translate-x-1/2 -translate-y-1/2 hover:cursor-zoom-out"
          src={imageSrc}
          alt={alt}
          onClick={() => {
            setModalOpen((curr) => !curr);
